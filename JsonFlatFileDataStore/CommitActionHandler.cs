    ...
    internal static class CommitActionHandler
{
    ...
    internal static async Task HandleStoreCommitActions(CancellationToken token, BlockingCollection<DataStore.CommitAction> updates,
        Action<bool> setExecutingState, Func<string, Task<bool>> updateState, Func<Task<string>> getLatestJson)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            var jsonText = await getLatestJson();
            ...
            try
            {
                updateSuccess = await updateState(jsonText);
            }
            catch (Exception e)
            {
                actionException = e;
